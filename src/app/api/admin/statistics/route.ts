...
export async function GET(request: NextRequest) {
  try {
    ...
    const [queries24h, queries7d, queries30d, topQueries] = await Promise.all([
      prisma.queryLog.count({ where: { createdAt: { gte: last24h } } }).catch(() => 0),
      prisma.queryLog.count({ where: { createdAt: { gte: last7d } } }).catch(() => 0),
      prisma.queryLog.count({ where: { createdAt: { gte: last30d } } }).catch(() => 0),
    ...
    const [activeUsers24h, activeUsers7d, activeUsers30d, newUsers7d, newUsers30d] =
      await Promise.all([
        prisma.user.count({
          where: {
    ...
    const topQueriesFormatted = topQueries.map((item) => ({
      ...
      lastQueriedAt: item._max.createdAt?.toISOString() || new Date().toISOString(),
    }))
    ...
    const avgIndexingTime = completedJobs.length > 0
      ? Math.round(
