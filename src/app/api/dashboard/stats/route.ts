...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get workspace sources (only sources added by the user, not global)
    const sources = await prisma.source.findMany({
      where: {
        scope: "WORKSPACE", // Only count workspace sources
        workspaceSources: {
          some: {
    ...
    // Get recent activity (last 10 jobs) - only for user's workspace sources
    const recentJobs = await prisma.job.findMany({
      where: {
        source: {
          scope: "WORKSPACE", // Only show activity for workspace sources
          workspaceSources: {
            some: {
              workspaceId: workspace.id,
            },
