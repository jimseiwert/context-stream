...
export async function POST(
  ...
  try {
    ...
    // Queue the scraping job (re-scrape uses batch embeddings for 50% cost savings)
    await addScrapeJob(id, { isInitialScrape: false });
    ...
    const { id } = await params;
    console.error(`[API] POST /api/sources/${id}/scrape error:`, error);
