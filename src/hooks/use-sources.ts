  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.isGlobal || data.source?.scope === "GLOBAL") {
        toast.success(data.message || "Global source added to workspace instantly!")
      } else {
        toast.success("Source created! Indexing started.")
      }
    },
    onError: (error: any) => {
      const message = error.data?.error || error.message || "Failed to create source"
      toast.error(message)
    },
  })
}
...
export function useUpdateSource(sourceId: string) {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      const message = error.data?.error || error.message || "Failed to update source"
      toast.error(message)
    },
  })
}
...
export function useDeleteSource() {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      const message = error.data?.error || error.message || "Failed to delete source"
      toast.error(message)
    },
  })
}
...
export function useRescrapeSource() {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      const message = error.data?.error || error.message || "Failed to start re-scraping"
      toast.error(message)
    },
  })
}
