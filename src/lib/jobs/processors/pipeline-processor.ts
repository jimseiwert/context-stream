...
export class PipelineProcessor {
  ...
  async process(): Promise<{ completed: number; failed: number }> {
    ...
    const currentSource = await db.query.sources.findFirst({
      where: eq(sources.id, this.config.sourceId),
  ...
  private async purgeStalePages() {
    try {
      ...
      const existingPages = await db
        ...
        .from(pages)
        .where(eq(pages.sourceId, this.config.sourceId));
      ...
      const deleted = await db
        .delete(pages)
        .where(inArray(pages.id, stalePageIds))
        .returning({ id: pages.id });

      console.log(
        `[Pipeline] ✓ Purged ${deleted.length} stale pages from database`
      );
    } catch (error: any) {
    ...
    while (this.isProcessing && !this.isCancelled) {
      ...
      try {
        ...
        if (this.pagesCheckedForMetadata < this.MAX_PAGES_TO_CHECK_FOR_METADATA) {
          ...
          if (this.pagesCheckedForMetadata >= this.MAX_PAGES_TO_CHECK_FOR_METADATA) {
            const finalSource = await db.query.sources.findFirst({
              where: eq(sources.id, this.config.sourceId),
              columns: { logo: true },
            });
    ...
    while (this.isProcessing && !this.isCancelled) {
      ...
      try {
        ...
        const existingPage = await db.query.pages.findFirst({
          where: and(
            eq(pages.sourceId, this.config.sourceId),
            eq(pages.url, task.url)
          ),
