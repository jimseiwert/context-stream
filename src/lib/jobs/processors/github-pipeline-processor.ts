...
export interface GitHubTask {
  file: GitHubFile
  stage: 'QUEUED' | 'FETCHING' | 'EMBEDDING' | 'SAVING' | 'COMPLETED' | 'FAILED'
  content?: string
  error?: string
}
...
export class GitHubPipelineProcessor {
  ...
  async process(): Promise<{ completed: number; failed: number }> {
    ...
    console.log(`[GitHub Pipeline] Completed: ${stats.completed}, Failed: ${stats.failed}`)
  ...
  private async fetchWorker(owner: string, repo: string, branch?: string) {
    while (this.isProcessing) {
      ...
      try {
        ...
        console.error(`[GitHub Pipeline] Fetch failed for ${task.file.path}:`, error.message)
        task.stage = 'FAILED'
    ...
    while (this.isProcessing) {
      ...
      try {
        ...
        if (task.content && task.content.length > 50) {
          const chunks = await this.embeddingProvider.chunkAndEmbed(task.content)
          console.log(`[GitHub Pipeline] Generated ${chunks.length} chunks for ${task.file.path}`)
        ...
        console.error(`[GitHub Pipeline] Embed failed for ${task.file.path}:`, error.message)
        task.stage = 'FAILED'
        task.error = error.message
        this.reportProgress()
      }
  ...
  private async saveWorker() {
    while (this.isProcessing) {
      ...
      try {
        ...
        const title = task.file.path.split('/').pop()?.replace(/\.[^.]+$/, '') || task.file.path
        ...
        console.error(`[GitHub Pipeline] Save failed for ${task.file.path}:`, error.message)
        task.stage = 'FAILED'
        task.error = error.message
        this.reportProgress()
      }
