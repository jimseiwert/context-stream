...
export class HybridSearch {
  ...
  private async vectorSearch(query: string, sourceIds: string[]) {
    try {
      ...
      const embeddings = await this.embeddingProvider.generateEmbeddings([query])
...
export async function getFilteredSources(
  ...
  for (const source of sources) {
    ...
    if (parsed.frameworks.length > 0) {
      ...
      const tagMatch =
        source.tags &&
        source.tags.some((tag) => frameworkTags.includes(tag))
...
export async function hybridSearch(
  ...
  const ftsResults = excludePageIds.length > 0
  ...
  try {
    const embeddings = await embeddingProvider.generateEmbeddings([searchQuery])
    const queryVector = embeddings[0]
    const embeddingStr = `[${queryVector.join(',')}]`

    vectorResults = excludePageIds.length > 0
  ...
  const k = 60
  const scores = new Map<string, { text: number; vector: number; combined: number }>()
