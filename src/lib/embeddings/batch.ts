...
import { chunks as chunksTable } from '@/lib/db/schema';
import { getActiveEmbeddingConfig, type EmbeddingConfig } from './config';

/**
 * Build an OpenAI client from a decrypted embedding config.
 * Works for both OPENAI and AZURE_OPENAI providers.
 */
function buildOpenAIClient(config: EmbeddingConfig): OpenAI {
  const cc = config.connectionConfig as Record<string, string | undefined>
  const apiKey = cc.apiKey
  if (!apiKey) {
    throw new Error(`connectionConfig.apiKey is required for ${config.provider} batch API`)
  }
  return new OpenAI({
    apiKey,
...
export async function createBatchEmbeddingJob(sourceId: string): Promise<string> {
  ...
  const pages = await db
    .select({ id: pagesTable.id, contentText: pagesTable.contentText })
    .from(pagesTable)
    .where(and(eq(pagesTable.sourceId, sourceId), ne(pagesTable.contentText, '')));
    ...
    for (const [pageId, chunks] of batch) {
      try {
        ...
        if (chunkRecords.length > 0) {
          await db.insert(chunksTable).values(chunkRecords);
